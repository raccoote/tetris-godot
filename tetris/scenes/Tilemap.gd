...
var move_direction := 0  # -1 for left, 1 for right, 0 for none
...
func hard_drop():
	while can_move(Vector2i(0, 1)):
		current_piece.position.y += 1
	draw_piece(current_piece.name, current_piece.rotation, current_piece.position)
	spawn_new_piece()

func is_at_bottom(piece) -> bool:
	var shape = TETROMINOS[piece.name][piece.rotation]
	for tile in shape:
		var world_pos = piece.position + tile
		var below_pos = world_pos + Vector2i(0, 1)

		# Check if we're at the bottom of the board
		if below_pos.y >= 20:  # Assuming 20 rows (0 to 19)
			return true

		# Check if the cell below is already filled
		if get_cell_source_id(below_pos) != -1:
			return true
...
func draw_piece(piece_name: String, rotation: int, pos: Vector2i):
	...
	for tile in shape:
		set_cell(pos + tile, atlas, TETROMINO_COLORS[piece_name])
...
func can_move(offset: Vector2i) -> bool:
	...
	for tile in shape:
		var pos = current_piece.position + tile + offset
		if pos.y >= 20 or pos.x < 0 or pos.x >= 10 or get_cell_source_id(pos) != -1:
			return false
	return true
	
func spawn_new_piece():
	var shape_names = TETROMINO_COLORS.keys()
	var random_name = shape_names[randi() % shape_names.size()]
	current_piece = {
		"name": random_name,
		"rotation": 0,
		"position": Vector2i(4, 0)  # Start near top middle
	}	
